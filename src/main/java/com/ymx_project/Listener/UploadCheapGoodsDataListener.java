...
import com.ymx_project.entity.CheapGoods;
import com.ymx_project.repository.CheapGoodsRepository;
...
@Slf4j
public class UploadCheapGoodsDataListener implements ReadListener<CheapGoods>{
// 有个很重要的点 DemoDataListener 不能被spring管理，要每次读取excel都要new,然后里面用到spring可以构造方法传进去
        ...
        private List<CheapGoods> cachedDataList = ListUtils.newArrayListWithExpectedSize(BATCH_COUNT);
        /**
         * 假设这个是一个DAO，当然有业务逻辑这个也可以是一个service。当然如果不用存储这个对象没用。
         */
        private CheapGoodsRepository cheapGoodsRepository;


        /**
         * 如果使用了spring,请使用这个构造方法。每次创建Listener的时候需要把spring管理的类传进来
         *
         * @param cheapGoodsRepository
         */
        public UploadCheapGoodsDataListener(CheapGoodsRepository cheapGoodsRepository) {
            this.cheapGoodsRepository = cheapGoodsRepository;
        }
         ...
         */
        @Override
        public void invoke(CheapGoods data, AnalysisContext context) {
...
//            log.info("{}条数据，开始存储数据库！", cachedDataList.size());
            Iterator<CheapGoods> it = cachedDataList.iterator();
            while(it.hasNext()) {
                CheapGoods cheapGoods = it.next();
                cheapGoods.setAsinLink(asinToJavaData);
