...
import com.ymx_project.repository.RemoveARepository;
...
@Slf4j
public class UploadRemoveADataListener implements ReadListener<RemoveA> {
    ...
    private List<RemoveA> cachedDataList = ListUtils.newArrayListWithExpectedSize(BATCH_COUNT);
    /**
     ...
     *
     * @param removeARepository
     */
    public UploadRemoveADataListener(RemoveARepository removeARepository) {
        this.removeARepository = removeARepository;
    }
     ...
     */
    @Override
    public void invoke(RemoveA data, AnalysisContext context) {
...
//        log.info("removeA{}条数据，开始存储数据库！", cachedDataList.size());
        Iterator<RemoveA> it = cachedDataList.iterator();
        while(it.hasNext()) {
            RemoveA removeA = it.next();
            removeA.setAsinLink(asinToJavaData);
            removeARepository.save(removeA);
        }
        log.info("存储数据库成功！");
    }
