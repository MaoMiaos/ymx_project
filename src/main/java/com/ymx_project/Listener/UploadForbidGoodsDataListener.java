...
import com.ymx_project.entity.ForbidGoods;
...
@Slf4j
public class UploadForbidGoodsDataListener implements ReadListener<ForbidGoods> {
    ...
    private List<ForbidGoods> cachedDataList = ListUtils.newArrayListWithExpectedSize(BATCH_COUNT);
    /**
     * 假设这个是一个DAO，当然有业务逻辑这个也可以是一个service。当然如果不用存储这个对象没用。
     */
    private ForbidGoodsRepository forbidGoodsRepository;


    /**
     * 如果使用了spring,请使用这个构造方法。每次创建Listener的时候需要把spring管理的类传进来
     *
     * @param forbidGoodsRepository
     */
    public UploadForbidGoodsDataListener(ForbidGoodsRepository forbidGoodsRepository) {
        this.forbidGoodsRepository = forbidGoodsRepository;
    }
     ...
     */
    @Override
    public void invoke(ForbidGoods data, AnalysisContext context) {
    ...
    private void saveData() {
        log.info("forbidGoods{}条数据，开始存储数据库！", cachedDataList.size());
        Iterator<ForbidGoods> it = cachedDataList.iterator();
        while(it.hasNext()) {
            ForbidGoods forbidGoods = it.next();
            forbidGoods.setAsinLink(asinToJavaData);
