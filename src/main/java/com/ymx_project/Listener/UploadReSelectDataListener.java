...
import com.ymx_project.entity.ReSelect;
import com.ymx_project.repository.ReSelectRepository;
...
@Slf4j
public class UploadReSelectDataListener implements ReadListener<ReSelect> {
    ...
    private List<ReSelect> cachedDataList = ListUtils.newArrayListWithExpectedSize(BATCH_COUNT);
    /**
     ...
     *
     * @param reSelectRepository
     */
    public UploadReSelectDataListener(ReSelectRepository reSelectRepository) {
        this.reSelectRepository = reSelectRepository;
    }
     ...
     */
    @Override
    public void invoke(ReSelect data, AnalysisContext context) {
    ...
    private void saveData() {
        ...
        Iterator<ReSelect> it = cachedDataList.iterator();
        while(it.hasNext()) {
            ReSelect reSelect = it.next();
            reSelect.setAsinLink(asinToJavaData);
            reSelectRepository.save(reSelect);
        }
        log.info("存储数据库成功！");
    }
